...
void CompressiveTracker::classifierUpdateHyo(Mat& _sampleFeatureValue, vector<float>& _mu, vector<float>& _sigma, float _learnRate)
{
	Scalar muTemp;
	Scalar sigmaTemp;
	...
	for(int i = 0; i<featureNum; i++)
	{
		...
		_sigma[i] = sigmaTemp.val[0];
			
		_mu[i] = muTemp.val[0];	// equation 6 in paper
	}
}
...
void CompressiveTracker::classifierUpdate(Mat& _sampleFeatureValue, vector<float>& _mu, vector<float>& _sigma, float _learnRate)
{
	...
	for (int i=0; i<featureNum; i++)
	{
		meanStdDev(_sampleFeatureValue.row(i), muTemp, sigmaTemp);
		/*cout << i << endl;
		cout << muTemp.val[0] << "  " << sigmaTemp.val[0] << endl;
		cout << _mu[i] << "  " << _sigma[i] << endl;
...
void CompressiveTracker::adjustLearnRate()
{
	learnRate = 0.f;
}
...
void CompressiveTracker::init(Mat& _frame, Rect& _objectBox)
{
	...
	classifierUpdateHyo(samplePositiveFeatureValue, muPositive, sigmaPositive, learnRate);
	classifierUpdateHyo(sampleNegativeFeatureValue, muNegative, sigmaNegative, learnRate);
}
void CompressiveTracker::processFrameHyo(Mat& _frame, Rect& _objectBox)
{
	// predict

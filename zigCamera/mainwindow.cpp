...
mainWindow::mainWindow(QWidget *parent)
    : QWidget(parent)
{
    setWindowTitle(tr("Detracker By KCF"));
    setWindowIcon(QIcon(tr(":/images/sjtup")));
...
void mainWindow::setMainLayout()
{
    ...
    textLabel->setText(tr("The Elderly-Caring System"));
    ...
    descriptBtn = new QPushButton(tr("Description"));
    connect(descriptBtn, SIGNAL(pressed()), this, SLOT(descrip()));
    ...
    if(openVideo) {
        ...
        if(!frame.empty()) {
            if(start) {
                ...
                getBoundingBoxes(fgmask, boundingBoxes, 150000);
                ...
                mot.run(frame, boundingBoxes, true, true);
                mot.analyze();
...
void mainWindow::descrip()
{
    if(!descripPliay) {
        descripPliay = true;
        descriptBtn->setText(tr("Description"));
        totalNumEdit->clear();
        moveNumEdit->clear();
        fallEdit->setStyleSheet("background-color:rgba(255, 248, 220)");
    }
        ...
        descripPliay = false;
        descriptBtn->setText(tr("NO-Descript"));
    }
}
